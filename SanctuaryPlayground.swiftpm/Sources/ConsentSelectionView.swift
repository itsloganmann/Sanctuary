...
//  Sanctuary
//
//  Tinder-style card stack for expressing consent on boundary categories
//
...
struct BoundaryCard: Identifiable {
    ...
    static let defaultCards: [BoundaryCard] = [
        BoundaryCard(category: "Photos & Videos",     description: "Allow taking and sharing photos or videos together",     icon: "camera.fill",                          color: .purple),
        BoundaryCard(category: "Staying Overnight",    description: "Comfortable staying at each other's place",             icon: "moon.stars.fill",                      color: .indigo),
        BoundaryCard(category: "Physical Intimacy",    description: "Comfortable with physical affection",                   icon: "heart.fill",                           color: .pink),
        BoundaryCard(category: "Sharing Location",     description: "Share real-time location with each other",              icon: "location.fill",                        color: .safetyOrange),
        BoundaryCard(category: "Meeting Friends",      description: "Introduce each other to friend groups",                 icon: "person.3.fill",                        color: .blue),
        BoundaryCard(category: "Social Media Posts",   description: "Post about relationship on social media",               icon: "bubble.left.and.bubble.right.fill",    color: .cyan),
        BoundaryCard(category: "Alcohol Together",     description: "Comfortable drinking alcohol together",                 icon: "wineglass.fill",                       color: .red),
        BoundaryCard(category: "Private Space",        description: "Respect for personal time and space",                   icon: "house.fill",                           color: .green)
    ]
}
            ...
            if currentIndex >= boundaries.count {
                VStack(spacing: DesignTokens.spacingMedium) {
                    ...
                    Text("All Set!")
                        .font(.displaySmall)
                        .foregroundStyle(.white)
                    Text("Tap Review to see your choices")
                        .font(.bodyMedium)
                        .foregroundStyle(Color.textSecondary)
                }
            }
...
struct ConsentCardView: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel("\(card.category): \(card.description)")
        .accessibilityHint(isTop ? "Swipe right to consent, left to decline" : "")
    }
    
    private var strokeColor: Color {
        if offset.width > 30 { return .statusSafe }
        else if offset.width < -30 { return .statusDanger }
        else { return .borderSubtle }
    }
}
                ...
                Section("Consented") {
                    ForEach(boundaries.filter { $0.consent }) { card in
                        HStack {
                            ...
                            Text(card.category)
                            Spacer()
                            Image(systemName: "checkmark.circle.fill").foregroundStyle(Color.statusSafe)
                        }
                ...
                Section("Declined") {
                    ForEach(boundaries.filter { !$0.consent }) { card in
                        HStack {
                            ...
                            Text(card.category)
                            Spacer()
                            Image(systemName: "xmark.circle.fill").foregroundStyle(Color.statusDanger)
                        }
                        .accessibilityElement(children: .combine)
                    }
                }
